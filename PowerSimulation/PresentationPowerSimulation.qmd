...
sliderInput("Rel","Reliability of Measure", min=0, max=1, value = .8, step=.01)
  ...
  output$p1 <- renderPlot({
      ...
      ggplot(genDat(),aes(x=x)) + 
         ...
         geom_rect(aes(xmin = (mean(x) - 1.97*sd(x)/sqrt(length(x))),
                   ...
                   alpha = .01, fill = "pink") + 
      theme_xkcd() #+
      #annotate(paste("power = ",
      #               round(pwr.t.test(n = input$Ndemo, d = input$MUdemo/(sqrt((input$SDdemo^2)/input$Rel))), 2), sep=""), x = , y = , label = "Power")
  })
```

...
server <- function(input, output){
   ...
   #print(summary(model))
   output$p1 <- ggplot(x.l(),aes(x=T,y=value,colour=Measure)) + geom_smooth()
   output$p2 <- ggplot(x.l(),aes(x=T,y=value,colour=Measure)) + geom_smooth() + facet_wrap(~scen)
   ...
   output$p3 <- ggplot(dat.hat,aes(x=factor(G),y=T,colour=factor(R), ymin=T-2*T.se, ymax=T+2*T.se)) + geom_point(size=2) + facet_wrap(~U, labeller = labeller(U=Unc.labels)) + guides(col=guide_legend(title="Reliance")) + xlab("Goal Importance") + ylab("Overall Trust Rating (0-10)") + ylim(c(0,10)) + geom_errorbar(width=.2,col="black")
   output$p4 <- ggplot(x.l(),aes(x=as.factor(scen),y=value,colour=Measure))+geom_boxplot() + xlab("Scenario") + ylab("Level (0-10)")
   output$p5 <- grid.arrange(p1, p2, p3, p4)
}
